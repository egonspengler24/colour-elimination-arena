...
Status: **v1 deployed** at https://egonspengler24.github.io/colour-elimination-arena/ (repo: [egonspengler24/colour-elimination-arena](https://github.com/egonspengler24/colour-elimination-arena)). All 11 levels are in place and have been through two feedback rounds: an initial pacing pass, then a detailed per-level playtesting pass covering visual/structural rework of Levels 3, 4, 6, 7 and 9, plus general fixes (leg-title layout, seamless canvas-to-bins border, bucket texture, collection zone flush with the bins).

Update: a third round added a setup screen (colour selection and names, ball count, level order, starting speed, music: default / custom file / none), a game clock and per-leg timer, a live leaderboard with a leg log, and in-game speed buttons. The main loop now runs a fixed-step simulation clock so timers, pause and speed are consistent. Leg 4 (Vortex Well) was reworked: balls are released across the whole field and spiral in under a weak pull plus a stronger swirl, giving a ~38 s leg with a smooth arrival curve instead of a single burst.

**Known mechanical limit, not yet solved:** peg/gate-style levels (2, 7, 8, 9, 10) can't be slowed to Level 1's ~90s pace through gravity/obstacle tuning alone, because Level 1's pace comes from *periodic full blocking* (a ball can be made to wait, stationary, until a gap arrives) — peg fields only ever *deflect*, never fully stop a ball's descent, so lowering gravity just produces a slower version of the same fast completion rather than genuine waiting. Getting these to Level-1-scale durations would need an actual blocking mechanism (e.g. a closing gate that periodically seals shut), which is a design change, not a tuning one — worth a decision before pursuing further.

Still open: further pacing/difficulty tuning from real playtesting, music, colour customisation, speed controls (§8 deferred items), and the bucket texture could go further with a zoomed reference screenshot.
